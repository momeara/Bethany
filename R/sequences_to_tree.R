...
sequences_to_tree <- function(
	...
	if(class(sequences) == "character"){
		...
		sequences_output_fname <- paste0(
			...
			sequences_input_fname %>%
				...
				unlist %>%
				tail(1))
		file.copy(sequences_input_fname, sequenecs_output_fname)
		if(verbose){
  ...
  name_map <- sequences %>% seqinr::getName %>% sequence_name_to_uniprot_entry %>%
		make_tree_names(ser_agent_arg, verbose) %>%
	...
	cat(
		"#!/bin/sh\n\n",
		"### Autogenerated from phylogentic_pipeline.R ####\n",
		"### date: ", date_code(), "\n\n",
		sep="",
